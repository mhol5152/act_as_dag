module Dag
  module Edges
    ...
    # Class methods that extend the link model. Returns a new edge between two points
    ...
    def shortest_path_between(ancestor, descendant, path = [])
      ...
      ancestor.children.each do |child|
        if child == descendant
          temp = path.clone
          temp << child
          shortest = temp if shortest.blank? || temp.length < shortest.length
        elsif find_link(child, descendant)
          temp = path.clone
          temp << child
    ...
    # Instance methods included into the link model
    module EdgeInstanceMethods
      ...
      def destroyable!
        ...
        self[direct_column_name] = false if direct?
        true
      end
      ...
      def rewire_crossing(above_leg, below_leg)
        if above_leg.count_changed?
          ...
          unless below_leg.count_changed?
